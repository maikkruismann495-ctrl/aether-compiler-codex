...
class TypeChecker(Visitor):
    def __init__(self, ast: Program, function_table: Dict[str, Dict[str, Any]], classes: Dict[str, Dict[str, str]]):
        self.ast = ast
    ...
    def visit_VariableDecl(self, node: VariableDecl) -> None:
        ...
        if node.var_type:
            ...
            if exp != inf: raise TypeError(f"Cannot assign '{inf}' to '{exp}'.", node.line, node.col)
    ...
    def visit_Assignment(self, node: Assignment) -> None:
        if isinstance(node.target, Identifier):
            ...
            if not t: raise TypeError(f"Undeclared variable '{node.target.name}'.", node.line, node.col)
            node.target.inferred_type = t
        else:
            t = node.target.accept(self)
        v = node.value.accept(self)
        if t != v: raise TypeError(f"Cannot assign '{v}' to '{t}'.", node.line, node.col)
    ...
    def visit_IfStmt(self, node: IfStmt) -> None:
        if node.condition.accept(self) != "bool": raise TypeError("If condition must be bool.", node.line, node.col)
    ...
    def visit_ForStmt(self, node: ForStmt) -> None:
        if node.start.accept(self) != "int" or node.end.accept(self) != "int": raise TypeError("For bounds must be int.", node.line, node.col)
        p = self.current_scope; self.current_scope = TypeScope(p)
    ...
    def visit_WhileStmt(self, node: WhileStmt) -> None:
        if node.condition.accept(self) != "bool": raise TypeError("While condition must be bool.", node.line, node.col)
        node.body.accept(self)

    def visit_ReturnStmt(self, node: ReturnStmt) -> None:
        if self.current_function_return_type is None:
            if node.value: raise TypeError("Cannot return value from void function.", node.line, node.col)
        else:
            if not node.value: raise TypeError("Must return value.", node.line, node.col)
            if node.value.accept(self) != self.current_function_return_type: raise TypeError("Return type mismatch.", node.line, node.col)
    ...
    def visit_Identifier(self, node: Identifier) -> str:
        ...
        if not t: raise TypeError(f"Undeclared variable '{node.name}'.", node.line, node.col)
        node.inferred_type = t; return t

    def visit_MemberAccess(self, node: MemberAccess) -> str:
        obj_type = node.obj.accept(self)
        if obj_type not in self.classes: raise TypeError(f"Type '{obj_type}' has no members.", node.line, node.col)
        if node.member not in self.classes[obj_type]: raise TypeError(f"Class '{obj_type}' has no field '{node.member}'.", node.line, node.col)
    ...
    def visit_IndexAccess(self, node: IndexAccess) -> str:
        ...
        if not arr_type.endswith("[]"): raise TypeError(f"Cannot index type '{arr_type}'.", node.line, node.col)
        t = arr_type[:-2]
        node.inferred_type = t; return t

    def visit_BinaryOp(self, node: BinaryOp) -> str:
        l = node.left.accept(self); r = node.right.accept(self)
        if l != r: raise TypeError(f"Type mismatch in binary op '{node.op}'.", node.line, node.col)
        t = "bool" if node.op in ["==", "!=", "<", ">", "<=", ">=", "&&", "||"] else "int"
        node.inferred_type = t; return t

    def visit_UnaryOp(self, node: UnaryOp) -> str:
        t = node.right.accept(self)
        if node.op == "-" and t != "int": raise TypeError("Unary '-' requires int.", node.line, node.col)
        if node.op == "!" and t != "bool": raise TypeError("Unary '!' requires bool.", node.line, node.col)
        node.inferred_type = t; return t
    ...
    def visit_FunctionCall(self, node: FunctionCall) -> Optional[str]:
        ...
        for i, a in enumerate(node.args):
            ...
            if a_type != sig["params"][i]: raise TypeError(f"Arg {i+1} type mismatch.", node.line, node.col)
    ...
    def visit_MethodCall(self, node: MethodCall) -> Optional[str]:
        ...
        if full_name not in self.functions: raise TypeError(f"Method '{node.method}' not found on '{obj_type}'.", node.line, node.col)
        sig = self.functions[full_name]
        for i, a in enumerate(node.args):
            a_type = a.accept(self)
            if a_type != sig["params"][i]: raise TypeError(f"Arg {i+1} type mismatch.", node.line, node.col)
