...
class CodeGenerator(Visitor):
    ...
    def _get_path(self, node):
        ...
        if isinstance(node, IndexAccess):
            if not isinstance(node.index, Literal):
                self.engine.report(ErrorCodes.UNSUPPORTED_FEATURE, Severity.ERROR, "Array assignment indices must be literal integers for now.", node.line, node.col)
                return "#err err"
            arr_type, arr_loc = self._eval_expr(node.array)
            arr_obj = arr_loc.split()[-1]
            return f"{node.index.value} {arr_obj}"
    ...
    def visit_VariableDecl(self, node):
        if isinstance(node.value, ArrayLiteral):
            _, arr_obj = self.visit_ArrayLiteral(node.value)
            self.scopes[-1][node.name] = {"type": node.var_type.name, "player": "#elem", "obj": arr_obj}
            return
    ...
    def visit_CompoundAssign(self, node):
        ...
        if t in ["int", "bool"]:
            ops = {"+=": "+=", "-=": "-=", "*=": "*=", "/=": "/=", "%=": "%="}
            if node.op in ops:
                self.cmds.append(f"scoreboard players operation {p_loc} {ops[node.op]} {l}")
    ...
    def _eval_expr(self, node) -> Tuple[str, str]:
        if isinstance(node, Literal):
            ...
            return self.visit_ArrayLiteral(node)
            ...
            if node.op in ['+', '-', '*', '/', '%']:
                ...
                if node.op == '!=':
                    self.cmds.append(f"execute unless score {ll} = {rl} run scoreboard players set {res} 1")
                else:
                    mc_op = "=" if node.op == "==" else node.op
                    self.cmds.append(f"execute if score {ll} {mc_op} {rl} run scoreboard players set {res} 1")
                return ("bool", res)
            elif node.op == "&&":
                self.cmds.append(f"scoreboard players set {res} 0")
                self.cmds.append(f"execute if score {ll} matches 1 if score {rl} matches 1 run scoreboard players set {res} 1")
                return ("bool", res)
            elif node.op == "||":
                self.cmds.append(f"scoreboard players set {res} 0")
                self.cmds.append(f"execute if score {ll} matches 1 run scoreboard players set {res} 1")
                self.cmds.append(f"execute if score {rl} matches 1 run scoreboard players set {res} 1")
                return ("bool", res)
        elif isinstance(node, UnaryOp):
            rt, rl = self._eval_expr(node.right)
            res = self._alloc_temp(rt)
            if node.op == "-" and rt == "int":
                self.cmds.append(f"scoreboard players set {res} 0")
                self.cmds.append(f"scoreboard players operation {res} -= {rl}")
                return ("int", res)
            if node.op == "!" and rt == "bool":
                self.cmds.append(f"scoreboard players set {res} 0")
                self.cmds.append(f"execute if score {rl} matches 0 run scoreboard players set {res} 1")
                return ("bool", res)
            ...
            return (getattr(node, "inferred_type", None) or "int", "#ret ae_int")
        return ("unknown", "#err err")
    ...
    def visit_IndexAccess(self, node: IndexAccess):
        if not isinstance(node.index, Literal):
            self.engine.report(ErrorCodes.UNSUPPORTED_FEATURE, Severity.ERROR, "Array indices must be literal integers.", node.line, node.col)
            return ("int", "#err err")
        ...
        self.cmds.append(f"scoreboard players operation {res_loc} = {idx} {arr_obj}")
        return ("int", res_loc)
