...
class TypeChecker(Visitor):
    ...
    def visit_ReturnStmt(self, node: ReturnStmt) -> None:
        if self.current_function_return_type is None:
            ...
            elif node.value.accept(self) != self.current_function_return_type: self.engine.report(ErrorCodes.TYPE_MISMATCH, Severity.ERROR, "Return type mismatch.", node.line, node.col)
    ...
    def visit_ArrayLiteral(self, node: ArrayLiteral) -> str:
        ...
        for e in node.elements[1:]:
            et = e.accept(self)
            if et != t: self.engine.report(ErrorCodes.TYPE_MISMATCH, Severity.ERROR, f"Array element type '{et}' does not match '{t}'.", e.line, e.col)
    ...
    def visit_IndexAccess(self, node: IndexAccess) -> str:
        ...
        if idx_type != "int": self.engine.report(ErrorCodes.TYPE_MISMATCH, Severity.ERROR, "Array index must be int.", node.index.line, node.index.col)
        if not arr_type or not arr_type.endswith("[]"):
            self.engine.report(ErrorCodes.INVALID_OPERATION, Severity.ERROR, f"Cannot index type '{arr_type}'.", node.line, node.col)
            node.inferred_type = "unknown"; return "unknown"
        t = arr_type[:-2]
        node.inferred_type = t; return t
